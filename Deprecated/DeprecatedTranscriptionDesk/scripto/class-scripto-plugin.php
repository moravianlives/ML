			...
			<?php foreach ( $attachments as $attachment ): ?>
				...
				<figure class="gallery-item">
					<div class="gallery-icon portrait">
						<a data-post="<?php echo $params['scripto_doc_id']; ?>" data-image="<?php echo $params['scripto_doc_page_id']; ?>" href="<?php 
						//changed code below to use the same url to pull remote images for the thumbnails. This uses the full-size image and css scaling to display with the correct dimensions. This prevents the need to create thousands of small scale images of everything. If oringal image files are too large, this will cause slower load time. The css class below was also changed to only "attachment-thumbnail" with no extra spaces.
						echo site_url( '?' . http_build_query( $params ) ); ?>">
							<img width="150" height="150" src="<?php echo get_the_guid($attachment->ID); ?>" class="attachment-thumbnail">
						</a>
					</div>
				</figure>
