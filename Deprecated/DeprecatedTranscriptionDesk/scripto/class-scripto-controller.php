...
class Scripto_Controller
{
	...
	*/
	...
	public function get_media_viewer( $attachment_post_id ) {
		...
		//This is changed from the original to retrieve the guid value from wp_posts data table instead of the meta_value in the wp_postmeta table which is where wp_get_attachment_url gets its value from. WordPress uses this value to automatically generate an attachment url, which will be incorrect since we are using images hosted on another server. 
		$attachment_url = get_the_guid( $attachment_post_id );
...
*/
			$media_viewer = '<div class="image-viewer">'.
			//added separate css class for transcription images
				'<div class="image-container"><img src="'.$attachment_url.'" class="transcription-image"/></div>'.
